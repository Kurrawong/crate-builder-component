...
export class CrateManager {
  ...
  exportCrate() {
    ...
    let reverseConnectionsGroupedByTgtId = groupBy(
      this.properties,
      "tgtEntityId"
    );
      ...
      properties[property].forEach((instance) => {
        if (instance.tgtEntityId) {
          entity[property].push({
            "@id": this.__lookupEntityByDescriboId({
  ...
  getEntity({ id, describoId, loadProperties = true }) {
    ...
    if (describoId)
      entity = this.__lookupEntityByDescriboId({ id: describoId });
      ...
      if (isPlainObject(json[property])) {
        ...
        json[property].forEach((instance) => {
          if (isPlainObject(instance))
            flattened.push(this.flatten({ json: instance }));
        });
